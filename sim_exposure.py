...
PMMA_elastic_u = np.load(
    './elastic/final_arrays/PMMA/PMMA_'
...
for j in range(5):
    Si_electron_u[:, j] = np.load(
        './Akkerman_Si_5osc/u/u_' + str(j) + '_nm_precised.npy'
    )
...
PMMA_phonon_u = np.load(
    './simple_PMMA_MC/arrays_PMMA/PMMA_ph_IMFP_nm.npy'
)
